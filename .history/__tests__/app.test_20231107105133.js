...
describe("GET /api/songs", () => {
  ...
  test.only("returns songs with a range for danceability when danceability is a query", () => {
    return request(app)
    .get("/api/songs?danceability_min=0.320&danceability_max=0.630")
    .expect(200)
    .then( ( {body: { songs }}  )=> {
      console.log(songs, "songs")
      const danceabilitySongs = songData.filter(song => {
        console.log(song)
        console.log(song.danceability, "danceability")
        console.log(song.danceability >= 0.320 && songs.danceability <= 0.630)
        return song.danceability >= 0.320 && songs.danceability <= 0.630})
      console.log(danceabilitySongs)
      expect(songs).toHaveLength(danceabilitySongs.length)
      songs.forEach( (song) => {
        expect(song.danceability).toBeGreaterThanOrEqual(0.320)
        expect(normaliseDate(song.danceability)).toBeLessThanOrEqual(0.630)
      })
    })
