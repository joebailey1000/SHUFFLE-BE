...
describe("GET /api/songs", () => {
  ...
  test.only("returns songs with a given modenity when modernity is a query", () => {
    return request(app)
    .get("/api/songs?modernity=1.0")
    .expect(200)
    .then(({body: { songs }}) => {
      const modernity1Songs = songData.filter(song => {
        console.log(normaliseDate(song.release_date), "normalised")
        console.log(normaliseDate(song.release_date) === 1.00)
        return normaliseDate(song.release_date) === 1.00})
      console.log(modernity1Songs, "<<< MODERNITY 1 SONGS")
      expect(songs).toHaveLength(modernity1Songs.length)
      songs.forEach( (song) => {
        expect(song.modernity).toBe(1.0)
      })
    })
  })
