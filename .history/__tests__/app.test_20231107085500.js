...
describe("GET /api/songs", () => {
  ...
  test("returns songs with a given acousticness when acousticness is a query", () => {
    ...
    .expect(200)
    .then( ({body: { songs }}) => {
      const acousticnessSongs = songData.filter(song => song.acousticness === 0.000)
  ...
  test("returns songs with a given tempo when tempo is a query", () => {
    ...
    .expect(200)
    .then(({body: { songs }}) => {
      const tempoSongs = songData.filter(song => {
        console.log(song.tempo)
        return song.tempo === 120.045})
      console.log(tempoSongs)
      console.log(songs)
