...
exports.fetchSongsQuery = (query) => {
    ...
    if (!query.random) {
        ...
        songQueriesKeys.forEach((attribute) => {
            ...
            else if (songQueries[attribute]) conditions.push(`${attribute} = ${query[attribute]}`)
            
            if (songQueries[min] && songQueries[max]) conditions.push(`${attribute} BETWEEN ${query[min]} AND ${query[max]}`)
            else if (songQueries[min] && !songQueries[max]) conditions.push(`${attribute} >= ${query[min]}`)
            else if (!songQueries[min] && songQueries[max]) conditions.push(`${attribute} <= ${query[max]}`)
            else if (songQueries[attribute]) conditions.push(`${attribute} = ${query[attribute]}`) 
    })
