...
const seed = ({songData, userData}) => {
    ...
    .then(() => {
        const FormattedSongData = data.map((song) => {
        ...
        const queryString = format(`INSERT INTO songs (song_id,title, artist, modernity, popularity, danceability, energy, loudness, acousticness, instrumentalness, liveness, valence, tempo) VALUES %L RETURNING *;`, FormattedSongData)
        return db.query(queryString)
    })
    .then((data) => {
        const FormattedUserData = data.map((user) => {
            return [
                user.username,
