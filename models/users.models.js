...
exports.patchUserWeightings = (userId, weightings) => {
  ...
  return db.query(
    ...
    output_weightings=output_weigthings+$9
    WHERE user_id = $10
    RETURNING *;`,
    [
      popularity_weighting,
...
exports.postNewUserRatings = ({ user_id, song_id, ranking }, networkPrediction) => {
  return db.query(`INSERT INTO rankings (ranking_id, user_id, song_id, ranking, network_prediction) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [Date.now(), user_id, song_id, ranking, networkPrediction])
    .then(({ rows }) => rows)
}
